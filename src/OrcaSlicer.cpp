...
int CLI::run(int argc, char **argv)
{
    ...
    // Library previews are isolated geometry imports, before profiles or the GUI exist.
    if (argc > 1 && std::strcmp(argv[1], "--library-thumbnail") == 0) {
        if (argc != 5) {
            boost::nowide::cerr << "Library thumbnail: expected input, PNG output and scratch folder" << std::endl;
            return 2;
        }
        set_logging_level(0);
        boost::log::core::get()->set_logging_enabled(false);
        set_temporary_dir(argv[4]);
        tbb::global_control thumbnail_parallelism(tbb::global_control::max_allowed_parallelism, 2);
        std::string error;
        if (!ProjectThumbnail::generate(argv[2], argv[3], argv[4], error)) {
            boost::nowide::cerr << "Library thumbnail: " << error << std::endl;
            return 1;
        }
        return 0;
    }
    ...
    const auto resolve_cli_plate_config = [&](Slic3r::GUI::PartPlate *plate,
        ...
        PlateSlicingContext declared_appearance;
        if (const auto *names = m_print_config.option<ConfigOptionStrings>("filament_settings_id"))
            declared_appearance.filament_preset_names = names->values;
        ...
        if (!context.filament_preset_names.empty())
            PresetBundle::apply_plate_filament_colours(context, resolved);
        ...
        if (actual_vendor_id.empty() && resolved_printer_preset != nullptr && resolved_printer_preset->is_project_embedded) {
            if (const auto *vendor = resolved_printer_preset->config.option<ConfigOptionString>("printer_vendor_id"))
                actual_vendor_id = vendor->value;
        }
        if (actual_vendor_id.empty()) {
