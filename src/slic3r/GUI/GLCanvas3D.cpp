...
void GLCanvas3D::bed_shape_changed()
{
    ...
    //The camera is the user's; a bed swap must not move it. With per-plate printers a
    //plate click can change the bed shape, and the selected plate's grey-to-dark render
    //is the whole selection feedback. Paths that genuinely want a re-frame (startup,
    //project load, the view cube) request requires_zoom_to_bed/plate themselves.
    m_dirty = true;
}
...
void GLCanvas3D::reload_scene(bool refresh_immediately, bool force_full_scene_refresh)
{
    ...
    // Paint data is mutable only by painting, or by an explicit user deletion of a filament. Moving the
    // editing cursor to another plate, or reloading the scene, must never reach it. Upstream normalised
    // every volume's painting against the globally selected filament count here, on every scene reload,
    // because one project meant one printer and that count was a fact about the project. In this fork the
    // global filament list is the cursor's view of the CURRENT plate, so clicking a plate whose machine
    // carries one slot silently erased four-material paint from every object in the model, model-wide and
    // unrecoverably. A machine with fewer slots than an object is painted for is a slice-time translation
    // (see Print::object_extruders), not a licence to destroy what the user painted.
...
void GLCanvas3D::_render_imgui_select_plate_toolbar()
{
    ...
    if(m_sel_plate_toolbar.show_stats_item)
    {
        ...
        if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::UNSLICED || all_plates_stats_item->slice_state == IMToolbarItem::SliceState::SLICING) {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(bar_bg_bgn, bar_fg_end, IM_COL32(79, 135, 165, 255),   bar_size.y); // BAR PROGRESS
        }
        else if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::SLICE_FAILED) {
