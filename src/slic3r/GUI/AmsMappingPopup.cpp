...
void MaterialItem::doRender(wxDC& dc)
{
    ...
    if (m_selected)
    {
        dc.SetPen(wxPen(wxColour("#4F87A5"), FromDIP(2))); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void MaterialSyncItem::doRender(wxDC &dc)
{
    ...
    if (m_selected) {
        dc.SetPen(wxColour("#4F87A5")); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
        dc.DrawRoundedRectangle(1, 1, size.x - 1, size.y - 1, 5);
    }
#else
...
AmsMapingPopup::AmsMapingPopup(wxWindow *parent, bool use_in_sync_dialog) :
    PopupWindow(parent, wxBORDER_NONE | wxPU_CONTAINS_CONTROLS), m_use_in_sync_dialog(use_in_sync_dialog)
 {
     ...
     m_left_tip_text = _L("Select filament installed on the left nozzle");
     m_right_tip_text = _L("Select filament installed on the right nozzle");
...
{
    wxPaintDC dc(this);
    dc.SetPen(wxPen(StateColor::darkModeColorFor(wxColour("#4F87A5")), FromDIP(2))); // Orca: colorful accent border for separation (dark-mode aware)
    dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void AmsReplaceMaterialDialog::create()
{
    ...
    identical_filament->SetForegroundColour(StateColor::darkModeColorFor(wxColour("#4F87A5"))); // Orca: accent teal (not brand green)
    ...
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(63, 111, 138), StateColor::Pressed), std::pair<wxColour, int>(wxColour(79, 135, 165), StateColor::Normal)); // Orca: accent teal button (not brand green)
