    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_link_edit_nozzle->SetForegroundColour("#4F87A5");
    ...
    m_statictext_finish->SetForegroundColour(wxColour(79, 135, 165));
...
{
    if (enable) {
        m_ams_backup_tip->SetForegroundColour(StateColor::darkModeColorFor("#4F87A5"));
    }
    else {
...
void SelectMachineDialog::show_timelapse_storage_dialog(MachineObject* obj)
{
    ...
    if (show_confirm_btn) {
        ...
        StateColor confirm_bg(std::pair<wxColour, int>(wxColour(79, 135, 165), StateColor::Normal));
    ...
    if (!show_confirm_btn) {
        // Orca: use the accent green rather than the reference's hard-coded confirm colour.
        StateColor cancel_bg(std::pair<wxColour, int>(wxColour(79, 135, 165), StateColor::Normal));
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#4F87A5") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
